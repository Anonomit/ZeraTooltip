...
local MAJOR = "LibTipHooker-1.1-ZeraTooltip"
local MINOR = "$Revision: 17 $"
...
local Set = {
  ...
  spell = function(methodName, tooltip, spellId, bookType)
    for handler in pairs(HandlerList.spell) do
      handler(tooltip, methodName, spellId, bookType)
    end
  end,
  talent = function(methodName, tooltip, tabIndex, talentIndex)
    for handler in pairs(HandlerList.talent) do
      handler(tooltip, methodName, tabIndex, talentIndex)
    end
  end,
  unit = function(methodName, tooltip, unit)
    if not tooltip.GetUnit then return end
    local name = tooltip:GetUnit()
    for handler in pairs(HandlerList.unit) do
      handler(tooltip, methodName, unit)
    end
  end,
  action = function(methodName, tooltip, slot)
    for handler in pairs(HandlerList.action) do
      handler(tooltip, methodName, slot)
    end
  end,
}
...
local function InitializeHook(tipType)
  ...
  while tooltip do
    ...
    if pcall(function() assert(tooltip:GetObjectType() == "GameTooltip") end) then
      local name = tooltip:GetName()
      if name then
        for _, v in ipairs(TooltipList[tipType]) do
          if strfind(name, "^"..v.."%d*$") then
            ...
            for _, methodName in ipairs(MethodList[tipType]) do
              if type(tooltip[methodName]) == "function" then
                hooksecurefunc(tooltip, methodName, function(...) Set[tipType](methodName, ...) end)
              end
            end
            break
          end
    ...
    assert(pcall(function() tooltip = EnumerateFrames(tooltip) end), "Failed to enumerate frames. This is most likely caused by an addon calling wipe() on a frame.")
  end
  ...
  if name and frameType == "GameTooltip" then
    for tipType in pairs(HandlerList) do
      for _, v in ipairs(TooltipList[tipType]) do
        ...
        if strfind(name, "^"..v.."%d*$") and not HookedFrames[name] then

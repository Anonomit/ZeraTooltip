...
function Addon:RewordLine(tooltip, line, tooltipData)
  ...
  if cache then
    ...
    if not line.stat and embeddedStatLines[line.type] and (self:GetOption("allow", "reword") or self:GetOption("allow", "recolor")) then
      ...
      if isRatingBusterActive then
        local capturePattern = " |cff%x%x%x%x%x%x%(.-%)|r"
        local startI, endI = strFind(lowerText, capturePattern)
        while startI do
          tinsert(ratingBusterAdditions, {startI, endI})
          startI, endI = strFind(lowerText, capturePattern, endI+1)
        end
      end
      ...
      for stat, statInfo in pairs(self.statsInfo) do
        if statInfo.GetNormalName then
          ...
          while startI do
            ...
            for _, addition in ipairs(ratingBusterAdditions) do
              if addition[1] < startI and endI < addition[2] then
                isInRatingBusterAddition = true
                break
              end
            end
            if not isInRatingBusterAddition then

...
function Addon:RewordLine(tooltip, line, tooltipData)
  ...
  local realText = line.realText
  
  -- TODO: config options for indenting negative stats
  if textCache[realText] then
    line.rewordLeft, line.rewordRight = unpack(textCache[realText], 1, 2)
    return
  end
  ...
  -- check compatibility
  if realText ~= line.textLeftText then
  ...
  if Addon:GetOption("allow", "reword") then
    ...
    -- elseif line.newPrefix then
    --   if strFind(line.newPrefix, "^" .. ITEM_SPELL_TRIGGER_ONUSE) then
    --     text = "|TInterface\\Buttons\\UI-AttributeButton-Encourage-Up:16:16:2:-2|t " .. text
    --   end
    end
  end
  ...
  if text ~= realText then
    line.rewordLeft = text
  end
  
  if self:GetOption("cache", "text") then
    textCache[realText] = {line.rewordLeft, line.rewordRight}
  end
end
